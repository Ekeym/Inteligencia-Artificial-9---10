...
class SpamVerifier:
    ...
    def _domain_of(email_address: str) -> str:
        m = re.search(r"@([^> )]+)$", email_address or "")
    ...
    def _looks_like_suspicious_url(self, url: str) -> bool:
        d = self._url_domain(url)

        if not d:
            return False

        # Acortadores
        if d in self.url_shorteners:
            return True
    ...
    def evaluate(self, dataset: List[Email]) -> None:
        ...
        print("\n=== RESULTADOS (umbral={}) ===".format(self.threshold))
...
if __name__ == "__main__":
    # Config por defecto (puedes mover umbral/pesos aquí)
